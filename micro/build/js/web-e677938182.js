...
$('#searchInput').keydown(function(e){
    if(e.keyCode == 13)
    {
        ...
        window.location = '/productos/buscar/'+$('#searchInput').val();
    }
});
...
function searchsm() {
    if (!$('#searchInputsm').val())
        return;

    window.location = '/productos/buscar/'+$('#searchInputsm').val();
}

$('#searchInputsm').keydown(function(e){
    if(e.keyCode == 13)
    {
        if (!$('#searchInputsm').val())
            return;
...
function submit_email() {
    $.ajax({
        type: 'POST',
        url: '/customers/new-email',
        data: { email: $('#email').val() }
    }).done(function(data) {
        toastr.success('Recibiras promociones muy pronto', 'Listo! email ingresado.');
    });
};

function submit_email_blackfriday() {
    $.ajax({
        type: 'POST',
        url: '/customers/new-email',
        data: { email: $('#email_black').val() }
    }).done(function(data) {
        toastr.success('Recibiras promociones muy pronto', 'Listo! email ingresado.');
    });
};

function submit_email_small() {
    $.ajax({
        type: 'POST',
        url: '/customers/new-email',
        data: { email: $('#email_black_small').val() }
    }).done(function(data) {
        toastr.success('Recibiras promociones muy pronto', 'Listo! email ingresado.');
    });
};
...
slider.setup('masterslider', { width:1920, height:934, space:1, layout:'fullwidth',
   loop:true, preload:0, instantStartLayers:true, autoplay:true, overPause:false });
...
var sliderSmall = new MasterSlider();
sliderSmall.control('arrows' ,{insertTo:'#mastersliderSmall'});
sliderSmall.control('bullets');

sliderSmall.setup('mastersliderSmall' , {
    width:1920,
    height:934,
    space:5,
    view:'basic',
